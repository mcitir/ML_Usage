Scikit-learn python library has a `LinearRegression` class, which is very popular to find model fitting according to data set

```python
from sklearn.linear_model import LinearRegression

model = LinearRegression()
model.fit(x_values, y_values) # model fitting according to training data set

model.predict([ [127], [248] ]) # predicting using the model


```

`pandas` can be used to import `*.csv` files into python scripts. 
...
**data set:** bmi_and_life_expectancy.csv
```csv
